...
export function TotalPopulation(data, activeYear, dashboard, city = null) {
  const totalCount = data.reduce((acc, val) => {
    let { year, total, district } = val;

    if (year === parseInt(activeYear)) {
      if (dashboard === "Seniors" || dashboard === "Newly Homeless") {
        if (district !== "1+2") {
          acc += total;
        }
      } else {
