...
import { filter } from "components/Utilities/ListManipulation";

const PercentageDistrict = ({
  totalCatData,
  totalPopData,
  activeYear,
  dashboard,
}) => {
  const totalCat = totalCatData["total"];
  const totalPopulation = filter.TotalPopulation(
    totalPopData,
    activeYear,
    dashboard
  );

  const percentage = (totalCat / totalPopulation) * 100;
  return (
    <div>
...
export default PercentageDistrict;
...
//           if (
//             this.props.dashboard === "Seniors" ||
//             this.props.dashboard === "Newly Homeless"
//           ) {
//             if (district !== "1+2") {
//               return acc + total;
//             }
...
//   }
//   render() {
//     return <div>{this.state.mydata ? this.runTotal() : null}</div>;
//   }
// }
