...
import DashboardGrid from 'components/layouts/grids/Seniors'
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const prepData = categories.reduce((acc, curr) => {
        ...
        const style = seniorsUnshelteredStyling[type];
        let data = [];
        if (filterType === "Individuals") {
          data =
            fetchData[`${router.activeYear}/SeniorsSubpopulations`][
              "Individuals"
            ];
        } else {
          data = filter.filterList(
            fetchData[`${router.activeYear}/SeniorsSubpopulations`][type],
            "subpopulation",
            FILTER_COLUMNS
          );
