...
import { remove, subset } from "components/Utilities/ListManipulation/filter";
import {
  countyColor,
  backgroundStroke,
  lat,
...
export default class LLMap extends Component {
  ...
  highlightFeature = (feature) => {
    ...
    this.setState({
      curHover: feature.target.feature.properties.DISTRICT
        ? feature.target.feature.properties.DISTRICT
        : feature.target.feature.properties.CITYNAME,
    });
  ...
  render() {
    ...
    if (curClicked == 0) {
      ...
      if (cityNames[curClicked]) {
        ...
        clickedHeader =
          curClicked.charAt(0) + curClicked.slice(1).toLowerCase();
      }
    } else {
    ...
    return (
      <div className="allContent">
        {/* <h4>Welcome to the Riverside County Health Informatics Data Portal</h4>
                 ...
                 */}
        <h4 key={Math.random()}>
          {place} - Total Unsheltered Individuals: {tot}
        </h4>
        <div className="row">
