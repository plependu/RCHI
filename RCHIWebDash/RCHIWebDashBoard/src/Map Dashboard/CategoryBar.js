...
import { subset } from "components/Utilities/ListManipulation/filter";
...
export default class CategoryBar extends Component {
  ...
  filterData = () => {
    ...
    if (curClicked === 0)
      // if 0 sum up entire county
      filterData = rawData;
    else if (curClicked.length > 1)
      // if longer then a digit, its a city and sort based on name
      filterData = subset(rawData, "city", curClicked);
    // else its a district and get by district
    else filterData = subset(rawData, "district", curClicked.toString());

    var newData = { ...keys }; // initializes newData object for sums

    for (var x = 0; x < filterData.length; x++) {
      if (!newData[filterData[x].subpopulation])
        newData[filterData[x].subpopulation] = 0; // if subpopulation is not in constant adds it
      newData[filterData[x].subpopulation] +=
        filterData[x].interview + filterData[x].observation; // sums obs and int
    }

    const formatted = Object.keys(newData).map((x) => {
      return {
        // formats data for the nivo bar
        name: x,
        total: newData[x],
      };
    });
  ...
  render() {
    ...
    if (this.state.curClicked !== this.props.curClicked) {
      // if out of sync with dom, re gets data to display, triggering a rerender
      this.filterData();
    }
    return (
      // if data has not been received, return loader
        ...
        <ResponsiveBar
          ...
          axisBottom={{
            ...
            renderTick: ({ textAnchor, textBaseline, value, x, y }) => {
              ...
              const TRIM_LENGTH =
                MAX_LINE_LENGTH * MAX_LINES - LENGTH_OF_ELLIPSIS;
              const trimWordsOverLength = new RegExp(
                `^(.{${TRIM_LENGTH}}[^\\w]*).*`
              );
