...
class UnshelteredSupervisoryDistricts extends Component {
  ...
  reformatData(Tables) {
    ...
    Tables[router.formerYear + "/SubpopulationsByCity"] = expandOnField(
      Tables[router.formerYear + "/SubpopulationsByCity"],
      "district"
    );
    Tables[router.activeYear + "/SubpopulationsByCity"] = expandOnField(
    ...
    for (const key in Tables[router.formerYear + "/SubpopulationsByCity"]) {
      ...
      Tables[router.formerYear + "/SubpopulationsByCity"][key] = expandOnField(
        Tables[router.formerYear + "/SubpopulationsByCity"][key],
        "category"
      );
    }
    ...
    for (const key in Tables[router.activeYear + "/SubpopulationsByCity"]) {
      ...
      Tables[router.activeYear + "/SubpopulationsByCity"][key] = expandOnField(
        Tables[router.activeYear + "/SubpopulationsByCity"][key],
        "category"
      );
    }
  ...
  NavOnChangeHandler = (e, data) => {
    ...
    this.setState({
      ...
      districtDisplay: (
        <PageLayout
          currentDistrict={this.state.currentPage}
        ...
        {this.state.rendered ? (
          <Container style={{ width: "80%" }}>
            <TrendNavBar
              ...
              changed={this.NavOnChangeHandler}
            />
