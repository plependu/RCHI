...
export default class CityTables extends Component {
  ...
  async fetchSubpopulationData() {
    ...
    for (var i = 0; i < indata.length; i++) {
      if (!filterData.includes(indata[i]["subpopulation"])) {
        ...
        data[indata[i]["city"]][indata[i]["category"]][
          indata[i]["subpopulation"]
        ]["Interview"] = indata[i]["interview"];
        data[indata[i]["city"]][indata[i]["category"]][
          indata[i]["subpopulation"]
        ]["Observation"] = indata[i]["observation"];
        data[indata[i]["city"]][indata[i]["category"]][
          indata[i]["subpopulation"]
        ]["Total"] = indata[i]["total"];
      }
      // break;
    }
  ...
  createTable = (incity) => {
    ...
    for (
      var x = 0;
      x < this.state.curCities.length &&
      Object.keys(this.state.households).length > 0;
      x++
    ) {
      var maxIntTot = 0;
      var maxObsTot = 0;
      ...
      if (Object.keys(this.state.data).length > 0) {
        ...
        for (var i = 0; i < 4; i++) {
          tableRow = [];
          tableRow.push(
            <th colspan="2" rowspan={Titles[Catergories[i]].length}>
              {" "}
              {Catergories[i]}{" "}
            </th>
          );
          curTot =
            parseInt(
              this.state.data[curCity][Catergories[i]]["Total"]["Observation"]
            ) +
            parseInt(
              this.state.data[curCity][Catergories[i]]["Total"]["Interview"]
            );
          ...
          for (var j = 0; j < Titles[Catergories[i]].length; j++) {
            int = parseInt(
              this.state.data[curCity][Catergories[i]][
                Titles[Catergories[i]][j]
              ]["Interview"]
            );
            obs = parseInt(
              this.state.data[curCity][Catergories[i]][
            ...
            else table.push(<tr> {tableRow} </tr>);
            tableRow = [];
            a += 1;
          }
        ...
        tableRow.push(
          <th rowspan="53">
            {" "}
            Subpopulations <br />{" "}
            <strong style={{ fontSize: 8 }}>Interview Only</strong>{" "}
          </th>
        );
        ...
        for (var i = 0; i < SubPopHeaders.length; i++) {
          tableRow.push(
            <th rowspan={Titles[SubPopHeaders[i]].length + 1}>
              {" "}
              {SubPopHeaders[i]}{" "}
            </th>
          );
          ...
          for (var j = 0; j < Titles[SubPopHeaders[i]].length; j++) {
            int = parseInt(
              this.state.data[curCity]["Subpopulations"][
                Titles[SubPopHeaders[i]][j]
              ]["Interview"]
            );
            obs = parseInt(
              this.state.data[curCity]["Subpopulations"][
        ...
        tableRow.push(
          <td> {this.state.households[curCity]["Adults Only"]} </td>
        );
        ...
        tableRow.push(
          <td> {this.state.households[curCity]["Children Only"]} </td>
        );
        tableRow.push(<td> {0} </td>);
        ...
        tableRow.push(
          <td> {this.state.households[curCity]["Adults and Children"]} </td>
        );
        tableRow.push(<td> {0} </td>);
        ...
        tableRow.push(
          <th colspan="3"> Total Unsheltered Homeless Individuals</th>
        );
        ...
        <Grid.Row>
          ...
          <Grid.Column width={12}>
            ...
            >
              ...
              <thead>
                <tr>
                  <th id="TableTitle" colspan="7">
                    The Subpopulations of {this.state.curCities}
                  </th>
                </tr>
                <tr>
              ...
              <tbody>{this.createTable(this.state.curCities[0])}</tbody>
            </table>
          </Grid.Column>
        </Grid.Row>
      </Grid>
