...
1)The Gender & Ethnicity & Race counts are based on the veterans in the households with at least a child and adult.
    It does include veterans in the household with children

2)I am assuming that the data set "csv" file is correct and the "P_Child Yes No" column is correct.
    With this columns I can correctly see which veterans belong in a household with at least a child and Adult.
...
def total_number_of_persons():
    ...
    total_non_veterans = in_df.loc[lambda df: (df['Household Survey Type'] == 'Interview')\
        & (df['Veteran'] == 0)\
            & (df['P_Child Yes No'] == 'Yes')\
                & ((df['Children (under 18)'] == 1) | (df['Adult (over 24)'] == 1) | (df['Youth (18-24)'] == 1))\
